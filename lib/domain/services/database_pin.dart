...
class DatabasePin {
  ...
  static Future updateRateOfPin(String? pinID) async {
    double rating = 0;
    return await FirebaseFirestore.instance.collection('reviews').where('pinID', isEqualTo: pinID).get().then((query) {
      for (DocumentSnapshot documentSnapshot in query.docs) {
        Map<String, dynamic> reviewMap = documentSnapshot.data() as Map<String, dynamic>;
        Review review = Review.fromMap(documentSnapshot.id, reviewMap);
        rating = rating + review.totalRate;
      ...
      rating = rating / query.docs.length;
      DocumentReference docRef = FirebaseFirestore.instance.collection('pins').doc(pinID);
      rating = double.parse(rating.toStringAsFixed(2));
  ...
  static Future<Pin> newPin(
    ...
    Map<String, dynamic> initialReviewMap = Review.newReviewMap(review, newPin.id);

    DocumentReference initialReview = await FirebaseFirestore.instance.collection('reviews').add(initialReviewMap);
  ...
  static Future<bool> isPinOwner(Pin pin) {
    return FirebaseFirestore.instance
      ...
      DocumentReference docRef = FirebaseFirestore.instance.collection('pins').doc(pin.id);
  ...
  Future editPin(Pin pin) async {
    firebaseInstance.collection('pins').doc(pin.id).set(
