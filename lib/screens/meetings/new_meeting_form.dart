...
class NewMeetingFormState extends State<NewMeetingForm> with AutomaticKeepAliveClientMixin<NewMeetingForm> {
  ...
  void _addMeeting(BuildContext context, Meeting meeting) async {
    ...
    if (_selectedDate.isAfter(DateTime.now().add(const Duration(hours: 2)))) {
      returnString = await DatabaseMeeting().addMeeting(meeting);
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    TextFormField(
                      ...
                      validator: (text) => text!.isEmpty ? 'Описание обязательно' : null,
                      maxLines: 3,
                      decoration: const InputDecoration(
                        hintText: 'Описание',
